pipeline {
    ...
    stages {
        ...
        stage('Webpacker Install') {
            steps {
                sh '/usr/local/bin/docker-compose run --rm web_testjenkins gem install nokogiri --platform=ruby; bundle config set force_ruby_platform true; bin/rails webpacker:install'
            }
        }
